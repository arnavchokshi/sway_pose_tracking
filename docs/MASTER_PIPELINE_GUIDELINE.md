...
This document describes the **implemented** production video pipeline in `sway_pose_mvp`: stage order, data contracts, defaults, environment variables, and where behavior is defined in code. It is the **primary** pipeline narrative; `docs/PHASE13_LAB_STRATEGIES.md` details Phases 1–3 Lab strategies. All claims below are tied to the cited modules. **§21–§27** list every Lab schema field plus CLI, YAML, env-only, and module-constant parameters and how they affect the run.
...
| `sway/experimental_hooks.py`                               | In-pipeline optional hooks: GNN refine (`SWAY_GNN_TRACK_REFINE` → `sway/gnn_track_refine.py`), HMR mesh sidecar JSON |
| `sway/gnn_track_refine.py`                                 | Post-stitch track graph, edge-conditioned GAT, merge pass; default weights `models/gnn_track_refine.pt` when present |
...
**Choreography stance (see §2.4):** treat **pose_stride=1** (every frame) and **linear** interpolation defaults as locked for production; GSI is not a substitute for skipped pose frames on fast, complex movement.
...
### 2.4 Choreography cadence and interpolation (master verdict)

**The Verdict:** Kill the GSI Speed Cheat. Your choreography is too fast and complex to skip frames.

**The Lock:** Permanently lock in `pose_stride=1` (2D pose runs on **every single frame**). In the Lab, `pose_stride` is fixed at **1** (no UI control). Keep temporal interpolation on **linear** (the default) for box stride gaps (`SWAY_BOX_INTERP_MODE`), pose stride gap fill (`SWAY_POSE_GAP_INTERP_MODE`, only if you use `--pose-stride 2` outside the Lab), and export MP4 tweening (`SWAY_VIS_TEMPORAL_INTERP_MODE`). Optional GSI modes remain in the schema for experiments; they smooth **between** anchors—they do not replace full-frame pose for fast, dense movement.
...
| `--pose-stride`          | `1`      | `2` skips every other pose frame; gaps filled per §9.3                                                                                            |
...
- `**raw_tracks`**: `Dict[track_id, List[observations]]` — per-frame boxes, confidences; observations may include `is_sam_refined`, `segmentation_mask` (hybrid SAM). After `apply_post_track_stitching`, `**maybe_gnn_refine_raw_tracks**` may run when `**SWAY_GNN_TRACK_REFINE**` is on (`**gnn_track_refine_enabled()**` in `experimental_hooks.py`): it builds a trajectory graph, runs the relational GNN + structural prior, and may merge track IDs (mutates `raw_tracks` in place) before Phase 4 — §7.2.
...
### 4.4 Pipeline checkpoints (resume / branch)

Checkpoints are **versioned bundles** under the run `output_dir` (or a path passed to `--resume-from`). They store **pipeline state**, not only preview MP4s. Canonical boundary IDs (CLI `--stop-after-boundary` / `--expect-boundary`):


| Boundary ID     | After printed phase  | Payload (minimum)                                                                                                                                                                                                                       |
| --------------- | -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `after_phase_1` | 1                    | **YOLO-only** detections: per-frame `(xyxy, conf)` after NMS, **before** hybrid SAM and **before** tracker association; plus video geometry and detector fingerprint (`sway/checkpoint_io.py`, `run_phase1_yolo_only` in `tracker.py`). |
| `after_phase_3` | 3                    | `raw_tracks` after `apply_post_track_stitching` and `maybe_gnn_refine_raw_tracks` (§4.1).                                                                                                                                               |
| `after_phase_4` | 4                    | `raw_tracks`, `surviving_ids`, `tracking_results` (`raw_tracks_to_per_frame` output) for pose input.                                                                                                                                    |
| `after_phase_5` | 5                    | `all_frame_data_pre` (§4.3) and dependencies (`tracking_results`, `frames_stored` metadata as needed).                                                                                                                                  |
| `after_phase_8` | 8                    | Post–Tier A/B/C `**all_frame_data`** list.                                                                                                                                                                                              |
| `final`         | 9–11 (single bundle) | **No** separate checkpoints for phases 9 or 10. The `final` manifest covers smoothing, scoring (if enabled), 3D lift sidecars, and export artifacts written at run end.                                                                 |


**Phases 6–7** have **no** checkpoint; they run in memory between phase 5 and phase 8 with full **phase debug** logging (below).

**Bidirectional tracking** (`SWAY_BIDIRECTIONAL_TRACK_PASS`): if `--stop-after-boundary after_phase_1` is used, the pipeline runs **forward YOLO only** (no reverse pass) so the checkpoint is **unambiguously** raw forward detector output. Resuming from `after_phase_1` into tracking **disables** bidirectional merge for that run (log a warning). Full bidirectional remains available for full runs without a phase-1 stop.

**Load safety:** each bundle includes `checkpoint_manifest.json` (`schema_version`, `boundary_id`, video size/hash, params hash, YOLO weights path when relevant). Mismatches refuse load unless `--force-checkpoint-load` is set.

### 4.5 Phase debug logs (`phase_debug.jsonl`)

When `--phase-debug-jsonl <path>` is set (Pipeline Lab sets it by default for runs), each major phase appends **one JSON object per line** with at least:

- `kind`: `"phase_summary"`
- `phase`: printed phase label, e.g. `"1"`, `"3"`, `"6-7"`, `"9"`, `"10"`, `"11"`
- `wall_s`: wall seconds spent in that phase
- `debug`: small dict of counters (detection counts, track counts, prune tallies, etc.) — **no** large tensors

Optional artifacts under `output_dir/debug/` are written only when `SWAY_PHASE_DEBUG_FILES=1`.
...
| 5             | Pose                             | ViTPose+ (incl. Sapiens card → ViTPose-Base or TorchScript when set), RTMPose-L; visibility gating; temporal keypoint refine | `pose_estimator.py`, `rtmpose_estimator.py`, `temporal_pose_refine.py`, `main.py` |
...
Pipeline Lab **intent** stages (`PIPELINE_STAGES` in `pipeline_config_schema.py`: `crowd_control`, `handshake`, `pose_polish`, `cleanup_export`) are the four tabs in Config and the Run editor flowchart. Each tab shows fields whose schema **`phase`** is mapped via `_LEGACY_PHASE_TO_INTENT` (mirrored in the React helper `CONFIG_STAGE_SCHEMA_PHASES` / `fieldBelongsToConfigStage` in `pipeline_lab/web/src/lib/configPageUi.ts`): **Crowd** ← `detection`; **Handshake** ← `tracking`, `phase3_stitch`, `reid_dedup`; **Pose** ← `pose`, `smooth`; **Export** ← `post_pose_prune`, `scoring`, `export`. Hybrid SAM controls use schema `phase: tracking` (same tab as tracker / phase-3 stitch / re-ID), not a separate schema phase. **Printed Phase 4 (pre-pose pruning) has no Lab tab** — its YAML-tunable subset is master-locked (§8.1). **`type: info` schema rows** (long “Lock these down” copy) are **not rendered** in the Pipeline Lab React UI (Config + Run editor); operators see section blurbs, the **Effective Phases 1–3** panel, and this guideline for lock details. **`sway_phase13_mode`** is edited from the **Recipe baseline** strip and **Phase13EffectivePanel** on Config (`PHASE13_MODE_FIELD_ID` is filtered out of per-step field lists so it is not duplicated under Handshake).
...
These environment values are **fixed for production runs** after `params` YAML is applied (`apply_master_locked_detection_env` in `main.py`, sourced from `MASTER_LOCKED_DETECTION_ENV` in `sway/pipeline_config_schema.py`). The Pipeline Lab **does not expose** controls for them. The schema includes a read-only **`info_detection_master_locked`** row for API/tooling; the React Lab UI **omits** all `type: info` fields (see §5). Lab runs additionally call `freeze_lab_subprocess_detection_env` in `pipeline_lab/server/app.py` so the child process always gets this stack.
...
These values are **fixed** after `params` YAML (`apply_master_locked_hybrid_sam_env` in `main.py`, from `MASTER_LOCKED_HYBRID_SAM_ENV` in `sway/pipeline_config_schema.py`). Schema includes **`info_hybrid_sam_master_locked`** for tooling; the React Lab UI omits `type: info` rows (§5). Lab subprocesses also call `freeze_lab_subprocess_hybrid_sam_env` in `pipeline_lab/server/app.py`.
...
After `params` YAML, `**apply_master_locked_phase3_stitch_env()`** (`main.py`, from `MASTER_LOCKED_PHASE3_STITCH_ENV` in `sway/pipeline_config_schema.py`) fixes long-range merge and AFLink internal thresholds. Schema includes **`info_phase3_master_locked`** for tooling; the React Lab UI omits `type: info` rows (§5). `**sway_global_aflink_mode`** (neural vs. heuristic linker) stays configurable.
...
### 7.2 Optional GNN track refine (after Phase 3 stitching, before Phase 4)

When `**SWAY_GNN_TRACK_REFINE**` is truthy (`1`/`true`/`yes` per `**gnn_track_refine_enabled()**` in `experimental_hooks.py`), `**maybe_gnn_refine_raw_tracks**` calls `**gnn_refine_raw_tracks**` in `**sway/gnn_track_refine.py**` **after** `apply_post_track_stitching` returns and **before** `track_stats.json` export and phase previews.

**Behavior:** Non-empty trajectories become graph nodes (hand-crafted descriptors). Candidate undirected edges connect tracks that overlap in time or have frame gap ≤ `**SWAY_GNN_MAX_GAP**` (default **120**). Each edge carries 6-D geometric/temporal features plus a fixed **structural prior** logit. A **relational GNN** (edge-conditioned multi-head GAT stack + link MLP) outputs learned logits; `sigmoid(learned + prior_scale × prior)` drives union–find merges above `**SWAY_GNN_MERGE_THRESH**` (default **0.55**), with hard guards against obvious two-person overlap. **Weights:** If `**SWAY_GNN_WEIGHTS**` points to an existing `.pt` file, its `state_dict` is loaded (tensor shapes must match the current hidden/heads/layers or only compatible layers apply). If that env is unset and `**models/gnn_track_refine.pt**` exists under the repo root, that checkpoint is loaded the same way. Otherwise the network uses random initialization (prior still nudges decisions).

**Training (synthetic, offline):** `**python -m tools.train_gnn_track_refine --out models/gnn_track_refine.pt**` — BCE on same-person edge labels from generated fragments; see the tool’s docstring. **`SWAY_GNN_TRACK_SMOKE`** runs `tools/gnn_group_track_stub.py` for a tiny merge sanity check.
...
**Skeleton estimation cadence (master):** Pipeline Lab always runs **2D pose every frame** — `pose_stride` is fixed at `**1`** for Lab-launched runs (the stride control is **not** shown in the Lab UI; schema default + subprocess args stay aligned). This matches **§2.4** (every frame + linear interpolation by default). For batch scripts, matrices, or local experiments you can still pass `**--pose-stride 2`**; gap fill remains **§9.3**.

**Engineering override:** `SWAY_UNLOCK_POSE_TUNING=1` for smoke presets (`tools/validate_pipeline_e2e.py` sets 3D off and optional chunked ViTPose). With unlock, `SWAY_VITPOSE_SMART_PAD` is not forced; `vitpose_smart_pad_enabled()` in `pose_estimator.py` still defaults **on** unless you set `SWAY_VITPOSE_SMART_PAD=0` / `false` / `no` / `off`.
...
- **ViTPose crops (default):** `vitpose_smart_pad_enabled()` is **on** under the master stack (§9.0.1) — boxes are expanded with `smart_expand_bbox_xyxy` before inference; per-person pad passed into the estimator is **0** (expansion is already applied). If smart pad is **off** (`SWAY_VITPOSE_SMART_PAD` opt-out), the Phase 5 loop uses symmetric fractional padding: **0.15** baseline; **0.25** on large motion/size change; **0.10** when motion/size change is small.
...
**Production choreography:** Prefer **linear** (defaults) everywhere GSI is optional—see **§2.4**. GSI is not a substitute for running pose on every frame when motion is fast and intricate.
...
This section lists **every** field in `PIPELINE_PARAM_FIELDS` (`sway/pipeline_config_schema.py`), grouped by schema **`phase`** (the `phase` string on each field — **not** the same as `PIPELINE_STAGES[].id`; see §5 for the intent-stage map). Phase 4 pre-pose pruning is **not** in `PIPELINE_PARAM_FIELDS` — see §21.4 / §8.1. **Binding** meaning: `env` → subprocess `os.environ` (`SWAY_*`); `yaml` → written to run `params` only; `cli` → `main.py` CLI; `none` → display-only `info` or Lab-only logic in `pipeline_lab/server/app.py`; `reid_model_preset` → maps OSNet card to `SWAY_BOXMOT_REID_WEIGHTS`; `yaml_pruning_weight` → one entry inside the `PRUNING_WEIGHTS` dict in `params`.
...
### 21.2 Phase: `tracking` — Track IDs, Phases 1–3 strategy, overlap / hybrid SAM

All rows share schema `phase: tracking` (including hybrid SAM — there is **no** separate `hybrid_sam` phase). In the Lab they appear on the **Handshake** intent stage together with `phase3_stitch` and `reid_dedup` fields (§5).


| Lab field `id`                  | Binding           | Key / target                                                                                                           | Default     | Effect                                                                                                                                                                                                                                                                                    |
| ------------------------------- | ----------------- | ---------------------------------------------------------------------------------------------------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| *(info_tracking_backends)*      | none              | —                                                                                                                      | —           | After each frame knows where people are, this step decides “this box is still dancer 3.” Deep OC-SORT is tuned for dance. You can add track-time OSNet when outfits differ. Overlap / hybrid SAM controls live in the same schema phase (below).                                            |
...
| `sway_phase13_mode`             | env               | `SWAY_PHASE13_MODE`                                                                                                    | standard    | **Phases 1–3 strategy:** `standard`, `dancer_registry`, or `sway_handshake`. Also set from Recipe baseline on Config. Server overrides hybrid SAM env for `sway_handshake` (IoU **0.10**, weak cues off). See `docs/PHASE13_LAB_STRATEGIES.md`. *(choices: standard, dancer_registry, sway_handshake)* |
...
| `sway_gnn_track_refine`         | env               | `SWAY_GNN_TRACK_REFINE`                                                                                                | False       | When on, `main.py` runs post-stitch **GNN track refine** (`gnn_refine_raw_tracks`): graph over trajectories, optional weights from `models/gnn_track_refine.pt` or `SWAY_GNN_WEIGHTS` — §7.2. Advanced tuning: §24 (`SWAY_GNN_*`).                                                                                                                       |
...
| `sway_boxmot_reid_model`        | reid_model_preset | preset → `SWAY_BOXMOT_REID_WEIGHTS` in `app.py` when `tracker_technology` = `deep_ocsort_osnet`                        | osnet_x0_25 | OSNet checkpoint for track-time Re-ID. *(preset: `osnet_x0_25` → `models/osnet_x0_25_msmt17.pt`; when `tracker_technology` = `deep_ocsort_osnet`)*                                                                                                                                                              |
| *(info_hybrid_sam_master_locked)* | none            | —                              | —       | Schema-only copy: master locked hybrid SAM stack (§6.4.1): `SWAY_HYBRID_SAM_OVERLAP`, `SWAY_HYBRID_SAM_ROI_CROP`, `SWAY_HYBRID_SAM_ROI_PAD_FRAC`. Omitted in React Lab UI (§5).                                                                                                            |
| `sway_hybrid_sam_iou_trigger`     | env     | `SWAY_HYBRID_SAM_IOU_TRIGGER`  | 0.42    | Higher = only very overlapped pairs get fixed (faster, less segmentation). Lower = fix sooner (slower, cleaner lifts and partner work). With ROI crop on, lower values are cheaper than full-frame SAM. **Sway handshake** forces **0.10** at enqueue. *(min 0.25 max 0.65)* |
| `sway_hybrid_sam_weak_cues`       | env     | `SWAY_HYBRID_SAM_WEAK_CUES`    | False   | Optional Hybrid-SORT–style gate: after IoU says “maybe run SAM,” compare detections to the previous output. If the worst-overlap pair is height- and confidence-stable, SAM is skipped. **Handshake** forces off at enqueue.                                                                 |
...
### 21.3 Phase: `phase3_stitch` — Merge IDs
...
### 21.4 Phase 4 — Pre-pose pruning (removed from Lab; master-locked YAML)
...
### 21.5 Phase: `pose` — Body pose


| Lab field `id`                        | Binding | Key / target                | Default      | Effect                                                                                                                                                                                                                                                                                                                                                        |
| ------------------------------------- | ------- | --------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| *(info_pose_choreography_verdict)*    | none    | —                           | —            | **The Verdict:** Kill the GSI Speed Cheat; choreography is too fast and complex to skip frames. **The Lock:** `pose_stride=1` every frame in the Lab; keep interpolation **linear** (default) for box gaps, pose gap fill (CLI stride 2 only), and export tween — **§2.4**.                                                                                                                                                       |
...
| *(info_pose_alternates)*    | none    | —                           | —            | **Sapiens** is selectable as a pose card (runs ViTPose-Base until native Sapiens is wired). **GNN** track refine (post-stitch graph merge) toggle lives under **Tracking** (`sway_gnn_track_refine`). **HMR** mesh placeholder JSON is under Export. Regression tests: `python -m tools.golden_bench` from a terminal (not a Lab toggle).                                                                                     |
| `pose_stride`               | cli     | `pose_stride`               | **1**        | **Master / Lab:** skeleton estimation **every frame** — **§2.4**; field is **`lab_hidden`** in `pipeline_config_schema.py` (value forced to **1** in the UI; no visible control). `**--pose-stride 2**` remains valid for CLI, batch matrices, and tools. *(choices: 1, 2)*                                                                                                                    |
| `sway_pose_gap_interp_mode` | env     | `SWAY_POSE_GAP_INTERP_MODE` | linear       | Only when `**pose_stride`=2** (not offered in Lab UI). **Linear** (default) matches **§2.4**; GSI uses the same RBF smoother as optional box paths; lengthscale falls back to `SWAY_GSI_LENGTHSCALE` (master stack locks this to **0.35** — §6.2.1), or set `SWAY_POSE_GSI_LENGTHSCALE` in params YAML to override for pose gaps only. *(choices: linear, gsi)* |
| *(info_pose_master_locked)* | none    | —                           | —            | Schema-only: `SWAY_VITPOSE_MAX_PER_FORWARD` (unset = 0), `SWAY_VITPOSE_FP32`, `SWAY_VITPOSE_SMART_PAD`, `SWAY_3D_LIFT` — §9.0.1. Omitted in React Lab UI (§5).                                                                                                                                                                                             |
...
### 21.7 Phase: `post_pose_prune` — Skeleton cleanup
...
| `SWAY_UNLOCK_POSE_TUNING`                                                            | main.py                                       | When `1` / `true` / `yes`, skips `apply_master_locked_pose_env` so smoke tests can set `SWAY_3D_LIFT=0`, chunk ViTPose, force FP32, or omit the forced `SWAY_VITPOSE_SMART_PAD=1` (§9.0.1). Smart pad stays **on** by code default unless `SWAY_VITPOSE_SMART_PAD=0` / `false` / `no` / `off`.                                                                                  |
...
| `SWAY_PHASE13_MODE`                                                                  | main.py / handshake / dancer_registry modules | `standard` / `dancer_registry` / `sway_handshake`; Lab schema field `sway_phase13_mode` + `pipeline_lab/server/app.py` `_subprocess_env` overrides. See `docs/PHASE13_LAB_STRATEGIES.md`.                                        |
...
| `SWAY_VITPOSE_FP32 / SWAY_VITPOSE_MAX_PER_FORWARD / SWAY_VITPOSE_SMART_PAD`         | pose_estimator.py, main.py                    | ViTPose inference + smart bbox expansion before crops; master stack sets smart pad **on** after params (§9.0.1) unless `SWAY_UNLOCK_POSE_TUNING=1`. Opt out of smart pad with `SWAY_VITPOSE_SMART_PAD=0` / `false` / `no` / `off`. |
...
| `SWAY_GNN_MERGE_THRESH`                                                              | gnn_track_refine.py                           | Sigmoid threshold for merging a candidate edge (default **0.55**).                                                                                                                                                               |
| `SWAY_GNN_HIDDEN` / `SWAY_GNN_HEADS` / `SWAY_GNN_LAYERS` / `SWAY_GNN_DROPOUT`        | gnn_track_refine.py                           | GNN width, attention heads, depth, dropout; hidden must divide heads. Checkpoint load keeps only matching tensor shapes.                                                                                                        |
| `SWAY_GNN_MAX_GAP`                                                                   | gnn_track_refine.py                           | Max temporal gap (frames) for candidate edges between disjoint segments (default **120**).                                                                                                                                       |
| `SWAY_GNN_PRIOR_SCALE`                                                               | gnn_track_refine.py                           | Scales hand-crafted prior logits added to learned logits (default **1.0**).                                                                                                                                                    |
| `SWAY_GNN_WEIGHTS`                                                                   | gnn_track_refine.py                           | Explicit path to training checkpoint (`.pt` with `state_dict`). If unset, `models/gnn_track_refine.pt` is used when present.                                                                                                    |
| `SWAY_GNN_DEVICE`                                                                    | gnn_track_refine.py                           | `cpu`, `cuda`, or `auto` (default).                                                                                                                                                                                              |
| `SWAY_GNN_SEED`                                                                      | gnn_track_refine.py                           | If set, `torch.manual_seed` before GNN module construction (reproducibility).                                                                                                                                                    |


*Ad-hoc tools (not `main.py`):* `SWAY_HMR_SMOKE`, `SWAY_SAPIENS_SMOKE`, `SWAY_GNN_TRACK_SMOKE`, `SWAY_SAPIENS_HEATMAP_H/W`, `SWAY_SAPIENS_TORCHSCRIPT`. **GNN train:** `python -m tools.train_gnn_track_refine` (no env gate; writes `models/gnn_track_refine.pt` by default).
