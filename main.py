"""
Sway Pose Tracking V3.0 — Main Orchestrator
...
Production-ready, M2-optimized pipeline:
  ...
  3. Resolution-aware box pruning (20% duration, 5% bbox-height kinetic)
  ...
  8. Spatio-temporal scoring (circmean, cDTW, per-joint thresholds)
  9. Export & visualization (per-joint heatmap)
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Sway Pose Tracking V3.0 - Pose estimation for dance videos"
    )
    parser.add_argument(
        "video_path",
        type=str,
    ...
    print("\n[1/9] Running detection & tracking (YOLO11l + BoT-SORT @ native FPS, streaming)...")
    ...
    print("\n[2/9] Track pruning (20% duration, 5% bbox-height kinetic)...")
    ...
    print("\n[3/9] Running pose estimation (ViTPose-Large)...")
    ...
    print("\n[4/9] Occlusion re-ID + crossover refinement...")
    all_frame_data_pre = []
    ...
    # Phase 6: Smart mirror pruning (edge + inverted velocity + low lower-body conf)
    print("\n[5/9] Smart mirror pruning...")
    ...
    for fd in all_frame_data_pre:
        surviving_after_mirror.update(t for t in fd["track_ids"] if t not in mirror_prune_ids)
    if mirror_prune_ids:
    ...
    print("\n[6/9] Temporal smoothing (1 Euro filter)...")
    ...
    for i, fd_pre in enumerate(all_frame_data_pre):
        ...
        poses_filtered = {tid: data for tid, data in poses_raw.items() if tid not in mirror_prune_ids}
        boxes_filtered = [b for b, tid in zip(boxes, track_ids) if tid not in mirror_prune_ids]
        track_ids_filtered = [tid for tid in track_ids if tid not in mirror_prune_ids]
    ...
    print(f"\n[9/9] Done. Outputs in {output_dir}")
