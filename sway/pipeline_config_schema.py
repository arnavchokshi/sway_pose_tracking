...
PIPELINE_PARAM_FIELDS: List[Dict[str, Any]] = [
    ...
    # --- Future pipeline fields (PLAN modules) ---
    _f(
        "sway_tracker_engine",
        "detection",
        "Tracker engine",
        ...
        "solidtrack",
        binding="env",
        key="SWAY_TRACKER_ENGINE",
        choices=["solidtrack", "sam2mot", "sam2_memosort_hybrid", "memosort", "matr"],
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Primary tracker: solidtrack (BoxMOT default), sam2mot (SAM2 mask-based), or hybrid.",
    ),
    _f(
        ...
        "yolo26l_dancetrack",
        binding="env",
        key="SWAY_DETECTOR_PRIMARY",
        choices=["yolo26l_dancetrack", "rt_detr_l", "rt_detr_x", "co_detr", "co_dino"],
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Primary detector: YOLO (default) or NMS-free DETR variants (PLAN_02).",
    ),
    _f(
        ...
        False,
        binding="env",
        key="SWAY_DETECTOR_HYBRID",
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="YOLO every frame + DETR on overlap frames (PLAN_03).",
    ),
    _f(
        "sway_hybrid_overlap_iou_trigger",
        "detection",
        "Hybrid detector overlap IoU trigger",
        "float",
        0.30,
        binding="env",
        key="SWAY_HYBRID_OVERLAP_IOU_TRIGGER",
        min_=0.10,
        max_=0.65,
        advanced=True,
        tier=3,
        lab_hidden=True,
        description=(
            "When hybrid detector is ON: pairwise YOLO box IoU above this threshold triggers the "
            "precision DETR pass on that frame (PLAN_03). Distinct from SWAY_HYBRID_SAM_IOU_TRIGGER "
            "(BoxMOT hybrid SAM overlap refinement)."
        ),
    ),
    _f(
        ...
        False,
        binding="env",
        key="SWAY_ENROLLMENT_ENABLED",
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Enable dancer enrollment for closed-set re-ID (PLAN_07).",
    ),
    _f(
        "sway_coi_enabled",
        "detection",
        ...
        True,
        binding="env",
        key="SWAY_COI_ENABLED",
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Mask-level collision detection and memory quarantine (PLAN_05).",
    ),
    _f(
        "sway_pose_mask_guided",
        "pose",
        "Mask-guided pose",
        "bool",
        False,
        binding="env",
        key="SWAY_POSE_MASK_GUIDED",
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="SAM2 mask guidance for pose estimation with per-keypoint confidence (PLAN_17).",
    ),
    _f(
        ...
        "greedy",
        binding="env",
        key="SWAY_COLLISION_SOLVER",
        choices=["greedy", "hungarian", "dp"],
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Identity assignment solver at coalescence exit (PLAN_15).",
    ),
    _f(
        ...
        False,
        binding="env",
        key="SWAY_BACKWARD_PASS_ENABLED",
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Reverse-video pass for gap filling (PLAN_16).",
    ),
    _f(
        ...
        "",
        binding="env",
        key="SWAY_REID_PART_MODEL",
        choices=["", "bpbreid", "kpr", "bpbreid_finetuned"],
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Part-based re-ID model for fusion engine (PLAN_08/09).",
    ),
    _f(
        "sway_critique_dimensions",
        "export",
        "Critique dimensions",
        "text",
        "",
        binding="env",
        key="SWAY_CRITIQUE_DIMENSIONS",
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Comma-separated critique dimensions: formation,timing,extension,smoothness,sync (PLAN_19).",
    ),
    _f(
        "sway_lift_backend",
        "export",
        ...
        "motionagformer",
        binding="env",
        key="SWAY_LIFT_BACKEND",
        choices=["motionagformer", "motionbert"],
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="3D pose lifting backend: MotionAGFormer (default) or MotionBERT (PLAN_18).",
    ),
    _f(
        "sway_sam2_model",
        "detection",
        ...
        binding="env",
        key="SWAY_SAM2_MODEL",
        choices=["sam2.1_b", "sam2.1_l", "sam2.1_h"],
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="SAM2 checkpoint size: base (default), large, or huge. Larger = better masks, slower.",
    ),
    _f(
        ...
        "delete",
        binding="env",
        key="SWAY_COI_QUARANTINE_MODE",
        choices=["delete", "freeze"],
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="On collision: delete corrupted memory entries (default) or freeze them.",
    ),
    _f(
        "sway_reid_kpr_enabled",
        "reid_dedup",
        "KPR re-ID signal",
        "bool",
        False,
        binding="env",
        key="SWAY_REID_KPR_ENABLED",
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Enable keypoint-prompted re-ID signal in the fusion engine (PLAN_09).",
    ),
    _f(
        "sway_reid_w_part",
        "reid_dedup",
        "Re-ID weight: part appearance",
        "float",
        0.30,
        binding="env",
        key="SWAY_REID_W_PART",
        min_=0.0,
        max_=1.0,
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Weight for part-based appearance re-ID signal (PLAN_13).",
    ),
    _f(
        "sway_reid_w_kpr",
        "reid_dedup",
        ...
        tier=3,
        lab_hidden=True,
        description="Weight for keypoint-prompted re-ID signal (PLAN_13).",
    ),
    _f(
        "sway_reid_w_skeleton",
        "reid_dedup",
        "Re-ID weight: skeleton gait",
        "float",
        0.0,
        binding="env",
        key="SWAY_REID_W_SKELETON",
        min_=0.0,
        max_=1.0,
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Weight for skeleton gait re-ID signal. 0 = OFF (default in lean core).",
    ),
    _f(
        "sway_reid_w_face",
        "reid_dedup",
        ...
        0.0,
        binding="env",
        key="SWAY_REID_W_FACE",
        min_=0.0,
        max_=1.0,
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Weight for ArcFace re-ID signal. 0 = OFF (default in lean core).",
    ),
    _f(
        "sway_reid_w_color",
        "reid_dedup",
        "Re-ID weight: color histogram",
        "float",
        0.10,
        binding="env",
        key="SWAY_REID_W_COLOR",
        min_=0.0,
        max_=1.0,
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Weight for color histogram re-ID signal (PLAN_13).",
    ),
    _f(
        "sway_reid_w_spatial",
        "reid_dedup",
        "Re-ID weight: spatial prior",
        "float",
        0.05,
        binding="env",
        key="SWAY_REID_W_SPATIAL",
        min_=0.0,
        max_=1.0,
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Weight for spatial prior re-ID signal (PLAN_13).",
    ),
    _f(
        "sway_backward_stitch_min_similarity",
        "detection",
        "Backward stitch similarity",
        "float",
        0.60,
        binding="env",
        key="SWAY_BACKWARD_STITCH_MIN_SIMILARITY",
        min_=0.20,
        max_=0.95,
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Min multi-signal re-ID similarity to merge reverse track with forward dormant (PLAN_16).",
    ),
    _f(
        ...
        False,
        binding="env",
        key="SWAY_MOTE_DISOCCLUSION",
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Enable MOTE optical flow disocclusion matrix (PLAN_21).",
    ),
    _f(
        ...
        "raft_small",
        binding="env",
        key="SWAY_MOTE_FLOW_MODEL",
        choices=["raft_small", "raft_large"],
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Optical flow backbone for MOTE: raft_small (default) or raft_large.",
    ),
    _f(
        "sway_sentinel_sbm",
        "detection",
        ...
        False,
        binding="env",
        key="SWAY_SENTINEL_SBM",
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Enable Sentinel Survival Boosting Mechanism (PLAN_21).",
    ),
    _f(
        "sway_sentinel_grace_multiplier",
        "detection",
        "Sentinel grace multiplier",
        "float",
        3.0,
        binding="env",
        key="SWAY_SENTINEL_GRACE_MULTIPLIER",
        min_=1.0,
        max_=10.0,
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Grace period multiplier for Sentinel SBM track preservation.",
    ),
    _f(
        "sway_sentinel_weak_det_conf",
        "detection",
        "Sentinel weak detection conf",
        "float",
        0.08,
        binding="env",
        key="SWAY_SENTINEL_WEAK_DET_CONF",
        min_=0.01,
        max_=0.30,
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Weak detection confidence threshold for Sentinel SBM.",
    ),
    _f(
        "sway_umot_backtrack",
        "detection",
        ...
        False,
        binding="env",
        key="SWAY_UMOT_BACKTRACK",
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Enable UMOT Historical Backtracking for long dormancy recovery (PLAN_21).",
    ),
    _f(
        ...
        500,
        binding="env",
        key="SWAY_UMOT_HISTORY_LENGTH",
        min_=100,
        max_=2000,
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Number of history frames for UMOT backtracking.",
    ),
    _f(
        "sway_lift_multi_person",
        "export",
        "Multi-person 3D lift",
        "bool",
        False,
        binding="env",
        key="SWAY_LIFT_MULTI_PERSON",
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="MotionBERT multi-person shared floor plane + depth estimation (PLAN_18).",
    ),
    # ── Future-pipeline env keys (PLAN_01–21 closure) ──────────────────────
    _f(
        "sway_track_partial_mask_frac",
        "detection",
        "PARTIAL mask fraction",
        "float",
        0.30,
        binding="env",
        key="SWAY_TRACK_PARTIAL_MASK_FRAC",
        min_=0.10,
        max_=0.50,
        advanced=True,
        tier=3,
        lab_hidden=True,
        description=(
            "Mask area below this fraction of enrollment area → PARTIAL state (PLAN_01). "
            "Alias for SWAY_STATE_PARTIAL_MASK_FRAC. Sweep range 0.10–0.50."
        ),
    ),
    _f(
        "sway_track_dormant_mask_frac",
        "detection",
        "DORMANT mask fraction",
        "float",
        0.05,
        binding="env",
        key="SWAY_TRACK_DORMANT_MASK_FRAC",
        min_=0.01,
        max_=0.15,
        advanced=True,
        tier=3,
        lab_hidden=True,
        description=(
            "Mask area below this fraction → DORMANT state (PLAN_01). "
            "Alias for SWAY_STATE_DORMANT_MASK_FRAC. Sweep range 0.01–0.15."
        ),
    ),
    _f(
        "sway_track_max_age",
        ...
        lab_hidden=True,
        description=(
            "Frames in DORMANT before LOST. Future-pipeline alias for SWAY_BOXMOT_MAX_AGE. "
            "Sweep range 90–400."
        ),
    ),
    _f(
        ...
        True,
        binding="env",
        key="SWAY_BACKWARD_COI_ENABLED",
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Run Cross-Object Interaction during backward pass (LOCKED ON, PLAN_16).",
    ),
    _f(
        "sway_collision_dp_max_perms",
        "detection",
        "Collision DP max permutations",
        "int",
        120,
        binding="env",
        key="SWAY_COLLISION_DP_MAX_PERMUTATIONS",
        min_=24,
        max_=5040,
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Cap on DP permutation search for collision solver (default 120 = 5!, PLAN_15).",
    ),
    _f(
        "sway_detection_uncertain_conf",
        "detection",
        "Uncertain detection conf",
        "float",
        0.50,
        binding="env",
        key="SWAY_DETECTION_UNCERTAIN_CONF",
        min_=0.35,
        max_=0.65,
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Detections below this confidence are flagged uncertain (lower EMA weight).",
    ),
    _f(
        "sway_enrollment_gallery_signals",
        "reid_dedup",
        "Enrollment gallery signals",
        "text",
        "part,color,spatial",
        binding="env",
        key="SWAY_ENROLLMENT_GALLERY_SIGNALS",
        advanced=True,
        tier=3,
        lab_hidden=True,
        description=(
            "Comma-separated subset of {part,face,skeleton,color,spatial} "
            "to collect at enrollment (PLAN_07)."
        ),
    ),
    _f(
        "sway_enrollment_part_model",
        "reid_dedup",
        "Enrollment part model",
        "select",
        "bpbreid",
        binding="env",
        key="SWAY_ENROLLMENT_PART_MODEL",
        choices=["bpbreid", "paformer"],
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Part-based re-ID model for enrollment embeddings (PLAN_07).",
    ),
    _f(
        ...
        "coco17",
        binding="env",
        key="SWAY_POSE_KEYPOINT_SET",
        choices=["coco17", "wholebody133"],
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Keypoint format: coco17 (default) or wholebody133 with RTMW models (PLAN_17).",
    ),
    _f(
        "sway_pose_smart_pad",
        "pose",
        ...
        True,
        binding="env",
        key="SWAY_POSE_SMART_PAD",
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Smart bbox expansion before pose crops (LOCKED ON, proven in sweep_v3).",
    ),
    _f(
        ...
        0.30,
        binding="env",
        key="SWAY_POSE_VISIBILITY_THRESHOLD",
        min_=0.15,
        max_=0.50,
        advanced=True,
        tier=3,
        lab_hidden=True,
        description="Keypoint confidence below this → NOT_VISIBLE (PLAN_17). Sweep range 0.15–0.50.",
    ),
]
