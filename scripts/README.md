...
**CLI tools** (run from repo root, e.g. `python -m tools.prefetch_models`): `tools.benchmark`, `tools.benchmark_trackeval`, `tools.run_sweep`, `tools.run_baseline_test`, `tools.run_trackeval_boxmot_ablation`, `tools.batch_run_for_review`, `tools.prefetch_models`, `tools.export_models`. Primary pipeline entrypoint remains `main.py` at repo root.
