"""
Multi-Signal Re-ID Fusion Engine (PLAN_23 — Layer 3)

Confidence-gated weighted ensemble that fuses six independent re-ID signals
into a single score per candidate identity match. Uses MLP router for dynamic
...
class ReIDQuery:
    ...
    track_id: int
    part_embeddings: Optional[object] = None
    kpr_embedding: Optional[np.ndarray] = None
    gait_embedding: Optional[np.ndarray] = None
    face_embedding: Optional[np.ndarray] = None
    color_histograms: Optional[Dict[str, np.ndarray]] = None
...
class ReIDFusionEngine:
    """Weighted ensemble of six re-ID signals with confidence gating and MLP routing."""
    ...
    def __init__(
        ...
        mlp_router: Optional[object] = None,
    ):
        ...
        self._last_signal_availability: Dict[str, bool] = {}
    ...
    def match(self, query: ReIDQuery, current_frame: int = 0) -> Tuple[int, float]:
        ...
        Uses MLP router (or rule-based fallback) for dynamic weight computation
        based on which signals are available and their confidence levels.
        """
        if not self.gallery:
        ...
        signal_availability = self._compute_signal_availability(query, skel_min)
        self._last_signal_availability = signal_availability
        ...
        for dancer in self.gallery:
            ...
            if signal_availability.get("part", False) and dancer.part_embeddings:
                try:
                    ...
                    signal_scores["part"] = score
                    signal_confidences["part"] = score
                except Exception:
                    pass
                ...
                if hasattr(query.part_embeddings, "global_emb"):
                    sim = float(np.dot(dancer.global_embedding, query.part_embeddings.global_emb))
                    score = max(0.0, sim)
                    signal_scores["part"] = score
                    signal_confidences["part"] = score

            # Signal 2: KPR (only during multi-person overlap)
            if signal_availability.get("kpr", False) and dancer.global_embedding is not None:
                sim = float(np.dot(dancer.global_embedding, query.kpr_embedding))
                score = max(0.0, sim)
                signal_scores["kpr"] = score
                signal_confidences["kpr"] = score

            # Signal 3: Skeleton gait
            if signal_availability.get("skeleton", False) and dancer.skeleton_gait_embedding is not None:
                sim = float(np.dot(dancer.skeleton_gait_embedding, query.gait_embedding))
                score = max(0.0, sim)
                signal_scores["skeleton"] = score
                signal_confidences["skeleton"] = score

            # Signal 4: Face
            if signal_availability.get("face", False) and dancer.face_embedding is not None:
                sim = float(np.dot(dancer.face_embedding, query.face_embedding))
                score = max(0.0, sim)
                signal_scores["face"] = score
                signal_confidences["face"] = score

            # Signal 5: Color histograms
            if signal_availability.get("color", False) and dancer.color_histograms:
                try:
                    ...
                    score = max(0.0, 1.0 - dist)
                    signal_scores["color"] = score
                    signal_confidences["color"] = score
                except Exception:
                    pass
            ...
            signal_confidences["spatial"] = spatial_score

            dynamic_weights = self._get_dynamic_weights(signal_availability, signal_confidences)
            self._last_dynamic_weights = dynamic_weights

            fused = 0.0
            for sig, score in signal_scores.items():
                w = dynamic_weights.get(sig, 0.0)
                fused += w * score
    ...
    def _compute_signal_availability(self, query: ReIDQuery, skel_min: int) -> Dict[str, bool]:
        """Determine which signals are available for this query."""
        return {
            "part": query.part_embeddings is not None,
            "kpr": query.is_multi_person_overlap and query.kpr_embedding is not None,
            "skeleton": (query.gait_embedding is not None and query.skeleton_window_length >= skel_min),
            "face": query.face_embedding is not None,
            "color": query.color_histograms is not None and len(query.color_histograms) > 0,
            "spatial": True,
        }
    ...
    def _get_dynamic_weights(
        ...
        """Get dynamically computed weights from MLP router or rule-based fallback."""
        if self.mlp_router is not None:
            try:
                return self.mlp_router.compute_weights(availability, confidences)
            except Exception:
                pass

        active_weights: Dict[str, float] = {}
        for sig, avail in availability.items():
            if avail:
                active_weights[sig] = self.weights.get(sig, 0.0)

        total = sum(active_weights.values())
        if total <= 0:
            return {s: 1.0 / 6 for s in self.weights}

        return {s: w / total for s, w in active_weights.items()}
