...
def vitpose_effective_max_per_forward(device: Union[torch.device, str]) -> int:
    ...
    if dev.type == "mps":
        try:
            # Default 8: balance between MPS memory and throughput (was 4, raised for speed).
            raw = int(os.environ.get("SWAY_VITPOSE_MPS_CHUNK", "4").strip())
        except ValueError:
            raw = 2
...
def vitpose_opencv_preprocess_enabled() -> bool:
    """
    Use direct OpenCV preprocessing instead of HuggingFace PIL-based processor.
    Faster for batch crops (avoids PIL round-trip). Same ImageNet normalization and
    bilinear resize to (256, 192). Default off; ``SWAY_VITPOSE_OPENCV_PREPROCESS=1`` enables.
    """
    v = os.environ.get("SWAY_VITPOSE_OPENCV_PREPROCESS", "0").strip().lower()
    return v in ("1", "true", "yes", "on")
...
class PoseEstimator:
    ...
    def __init__(
        ...
        self._use_opencv_preprocess = vitpose_opencv_preprocess_enabled()
        if self._use_opencv_preprocess:
            print("  ViTPose: OpenCV preprocessing enabled (SWAY_VITPOSE_OPENCV_PREPROCESS=1).", flush=True)
    ...
    # ImageNet normalization constants (same as HuggingFace ViTPose processor)
    _IMAGENET_MEAN = np.array([0.485, 0.456, 0.406], dtype=np.float32)
    _IMAGENET_STD = np.array([0.229, 0.224, 0.225], dtype=np.float32)

    def _preprocess_opencv(
        self,
        frame: np.ndarray,
        ...
        """
        Direct OpenCV preprocessing: crop, resize to (256, 192), normalize with ImageNet stats.
        Produces the same tensor layout as the HuggingFace processor but avoids PIL conversion
        and the slow Python-level image processor loop.

        Returns: (N, 3, 256, 192) float32 tensor on CPU.
        """
        img_h, img_w = frame.shape[:2]
        crops = []
        for box, pad in zip(boxes, paddings):
            ex1, ey1, ex2, ey2 = _expand_bbox(box, img_w, img_h, pad)
            ex1i, ey1i = max(0, int(ex1)), max(0, int(ey1))
            ex2i, ey2i = min(img_w, int(round(ex2))), min(img_h, int(round(ey2)))
            crop = frame[ey1i:ey2i, ex1i:ex2i]
            if crop.size == 0:
                crop = np.full((256, 192, 3), 114, dtype=np.uint8)
            resized = cv2.resize(crop, (192, 256), interpolation=cv2.INTER_LINEAR)
            # uint8 RGB -> float32 [0,1] -> ImageNet normalize -> CHW
    ...
    def _estimate_poses_batch(
        ...
        if self._use_opencv_preprocess:
            pixel_values = self._preprocess_opencv(frame, boxes, paddings)
            inputs = {"pixel_values": pixel_values}
            _vitpose_dbg(f"opencv preprocess done in {(time.perf_counter() - t0) * 1000:.1f}ms (CPU tensors)")
        else:
        ...
        for i, pose_dict in enumerate(image_pose_result):
            ...
            result[int(tid)] = {
                "keypoints": full_kpts,
                "scores": scores,
                "keypoints_vitpose_17": full_kpts.copy(),
                "fusion_source_17": ["vitpose"] * int(min(17, full_kpts.shape[0])),
            }
    ...
    def estimate_poses_with_cache(
        ...
        paddings: List[float] = None,
        segmentation_masks: Optional[List[Optional[np.ndarray]]] = None,
        prev_poses: Optional[Dict[int, Dict]] = None,
        bbox_displacements: Optional[Dict[int, float]] = None,
    ) -> Dict[int, Dict]:
        """
        Pose estimation with displacement-based caching: tracks whose bounding box moved
        less than a pixel threshold reuse the previous pose (shifted by the bbox delta)
        instead of running a new ViTPose forward pass.

        This is additive — when ``prev_poses`` or ``bbox_displacements`` are None, it
        falls back to the standard ``estimate_poses`` path.

        Args:
            frame: RGB image (H, W, 3), uint8.
            boxes: List of (x1, y1, x2, y2) in xyxy format.
            track_ids: List of track IDs, one per box.
            paddings: Optional per-box padding fractions.
            segmentation_masks: Optional SAM masks (same as ``estimate_poses``).
            prev_poses: Dict[track_id -> {"keypoints": (17,3), "scores": (17,)}] from
                the previous frame. Tracks present here *and* below the displacement
                threshold will reuse the cached pose.
            bbox_displacements: Dict[track_id -> float] giving the L2 center displacement
                (in pixels) of each track's bbox between the previous and current frame.

        Returns:
            Dict mapping track_id to {"keypoints": (17, 3), "scores": (17,)} — same
            format as ``estimate_poses``.
        """
        if len(boxes) == 0:
            return {}
        ...
        if prev_poses is None or bbox_displacements is None:
            return self.estimate_poses(frame, boxes, track_ids, paddings, segmentation_masks)

        try:
            thresh = float(os.environ.get("SWAY_POSE_CACHE_DISPLACEMENT_THRESH", "5.0").strip())
        except ValueError:
            thresh = 5.0
        ...
        for i, tid in enumerate(track_ids):
            disp = bbox_displacements.get(tid)
            if disp is not None and disp < thresh and tid in prev_poses:
                reuse_idx.append(i)
            else:
                run_idx.append(i)

        _vitpose_dbg(
            f"pose_cache total={len(boxes)} reuse={len(reuse_idx)} run={len(run_idx)} "
            f"thresh={thresh:.1f}"
        )

        out: Dict[int, Dict] = {}

        # Reuse previous poses with affine shift based on bbox center displacement
        for i in reuse_idx:
            tid = track_ids[i]
            prev = prev_poses[tid]
            prev_kpts = prev["keypoints"].copy()  # (17, 3)
            prev_scores = prev["scores"].copy()    # (17,)

            # Compute center shift between previous and current bbox.
            # prev_poses keypoints are in global image coords, so we shift by the
            # difference in bbox centers.
            cur_box = boxes[i]
            cur_cx = (cur_box[0] + cur_box[2]) * 0.5
            cur_cy = (cur_box[1] + cur_box[3]) * 0.5

            # Estimate previous center from keypoints (mean of visible joints)
            visible = prev_kpts[:, 2] > 0.3
            if np.any(visible):
                prev_cx = np.mean(prev_kpts[visible, 0])
                prev_cy = np.mean(prev_kpts[visible, 1])
                dx = cur_cx - prev_cx
                dy = cur_cy - prev_cy
            else:
                dx, dy = 0.0, 0.0
            ...
            prev_kpts[:, 0] += dx
            prev_kpts[:, 1] += dy
            entry = {"keypoints": prev_kpts, "scores": prev_scores}
            vk = prev.get("keypoints_vitpose_17")
            if isinstance(vk, np.ndarray) and vk.shape[0] >= 17 and vk.shape[1] >= 2:
                vks = vk.copy()
                vks[:, 0] += dx
                vks[:, 1] += dy
                entry["keypoints_vitpose_17"] = vks
            if "fusion_source_17" in prev:
                entry["fusion_source_17"] = list(prev.get("fusion_source_17") or [])
            out[tid] = entry
            _vitpose_dbg(f"pose_cache reuse tid={tid} dx={dx:.1f} dy={dy:.1f}")

        # Run normal pose estimation for tracks that moved significantly
        if run_idx:
            run_boxes = [boxes[i] for i in run_idx]
            run_tids = [track_ids[i] for i in run_idx]
            run_pads = [paddings[i] for i in run_idx]
            run_masks = (
                [segmentation_masks[i] for i in run_idx]
                if segmentation_masks is not None
                else None
            )
            out.update(
                self.estimate_poses(frame, run_boxes, run_tids, run_pads, run_masks)
            )
